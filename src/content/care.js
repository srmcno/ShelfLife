// Small, physical responses that only fit the resident receiving care.
export const TRAIT_CARE = {
  "damp": {
    "food": [
      "Dipped the dry food in itself.",
      "Ate, then left a grateful little tide mark."
    ],
    "fuss": [
      "Soaked into your sleeve a little. Affection, probably."
    ],
    "clean": [
      "You dried it. It is making more.",
      "The towel has developed a pulse. It wants joint custody.",
      "Stayed in the towel. Warmth without evaporation."
    ]
  },
  "porcelain": {
    "food": [
      "Clinked against the spoon. You both apologised."
    ],
    "fuss": [
      "Leaned into your hand very carefully."
    ],
    "clean": [
      "Polished up nicely. Can now see its own disapproval.",
      "You washed behind its ears. The glaze says dishwasher safe. You both know better."
    ]
  },
  "bitey": {
    "food": [
      "Bit the food instead of you. Progress.",
      "Chewed with its eyes shut. Finally, a victim that was meant for this."
    ],
    "fuss": [
      "Held your finger in its mouth without closing. A hug, locally."
    ],
    "clean": [
      "Bit the cloth. Let go when you asked."
    ]
  },
  "glitter": {
    "food": [
      "The food sparkles now. That cannot be undone."
    ],
    "fuss": [
      "Loved that. Your sleeve will remember longer than you."
    ],
    "clean": [
      "Clean pet. Festive sink."
    ]
  },
  "clingy": {
    "food": [
      "Ate from your hand so the hand would have to stay."
    ],
    "fuss": [
      "Wrapped itself round your thumb. You have a thumb warmer now."
    ],
    "clean": [
      "Held on through the rinse. Held on through the drying."
    ]
  },
  "fungal": {
    "food": [
      "Shared with the growth on its back. They seem close.",
      "Fed the mushroom first. You are caring for a household within a household."
    ],
    "fuss": [
      "Leaned in. A small mushroom leaned in with it."
    ],
    "clean": [
      "Came out clean. Its mushrooms came out offended."
    ]
  },
  "loadbearing": {
    "food": [
      "Ate without moving. The shelf thanks you."
    ],
    "fuss": [
      "Relaxed into it. The bracket made a noise."
    ],
    "clean": [
      "Lifted a corner for the cloth. The whole row held its breath."
    ]
  },
  "theatrical": {
    "food": [
      "Pretended to faint into the bowl. Ate from that position."
    ],
    "fuss": [
      "Bowed. Hit its head on the shelf above. Bowed lower."
    ],
    "clean": [
      "Played a drowning scene in a damp flannel. Took a bow."
    ]
  },
  "napoleon": {
    "food": [
      "Conquered the crumb. Ate the evidence."
    ],
    "fuss": [
      "Allowed you to pat the emperor. The emperor went floppy."
    ],
    "clean": [
      "Lost the battle of the flannel. Smells lovely."
    ]
  },
  "heirloom": {
    "food": [
      "Ate off the modern spoon. Survived the indignity."
    ],
    "fuss": [
      "Nestled into your palm. Apparently this is how it used to be done."
    ],
    "clean": [
      "The dust came off. The family resemblance stayed."
    ]
  },
  "swarm": {
    "food": [
      "Everyone got some. It took longer than the size suggested."
    ],
    "fuss": [
      "Purred in several directions."
    ],
    "clean": [
      "All clean. A faint round of applause from inside."
    ]
  },
  "undertaker": {
    "food": [
      "Laid the crumb to rest. In its mouth."
    ],
    "fuss": [
      "Went limp. Opened an eye to check you were impressed."
    ],
    "clean": [
      "Asked to be dried with dignity. Got the corner of a tea towel."
    ]
  },
  "haunted": {
    "food": [
      "Ate half. The other half went cold on its own.",
      "Two mouths ate. One bowl emptied."
    ],
    "fuss": [
      "Leaned into your hand. So did something a little to the left of it."
    ],
    "clean": [
      "Came out clean. The cold spot came with it, freshly rinsed."
    ]
  },
  "magpie": {
    "food": [
      "Ate the food. Kept the spoon. The spoon is in the stash."
    ],
    "fuss": [
      "Held your finger and checked it for rings."
    ],
    "clean": [
      "Was rinsed. Three small shiny things came out in the water. It wants them back."
    ]
  },
  "unblinking": {
    "food": [
      "Ate without looking down. Watched you the whole time."
    ],
    "fuss": [
      "Went soft under your hand. Its eyes stayed exactly where they were."
    ],
    "clean": [
      "You wiped its face. Its eyes were open for it. They are always open for it."
    ]
  },
  "sugar": {
    "food": [
      "Ate it in one go and asked, with its whole body, about pudding."
    ],
    "fuss": [
      "Vibrated under your thumb. That is a purr, at speed."
    ],
    "clean": [
      "Came out of the water sticky anyway. It is sticky from the inside."
    ]
  },
  "feral": {
    "food": [
      "Took the food to the back of the slot and ate it with its back to you. Growled, fondly."
    ],
    "fuss": [
      "Let you touch it. Once. Then bit the air where your hand had been, gently."
    ],
    "clean": [
      "Fought the cloth. Lost. Sat in the cloth afterwards, warm and sulking."
    ]
  },
  "amnesiac": {
    "food": [
      "Ate. Forgot it had. Sat by the bowl, hopeful, full."
    ],
    "fuss": [
      "Enjoyed it. Asked who you were halfway through. Kept leaning anyway."
    ],
    "clean": [
      "Was cleaned. Looked at itself, delighted, as if for the first time."
    ]
  },
  "hummer": {
    "food": [
      "Hummed into the bowl while it ate. The bowl rang a little."
    ],
    "fuss": [
      "Hummed under your thumb. You know the tune. You nearly had it."
    ],
    "clean": [
      "Hummed in the water. The tune came out clearer. Still eleven notes."
    ]
  },
  "insomniac": {
    "food": [
      "Ate slowly. It has time. It has all of it."
    ],
    "fuss": [
      "Closed its eyes under your hand. First time. Opened them at once, furious."
    ],
    "clean": [
      "Sat in the warm towel afterwards and nearly, nearly dropped off. Did not."
    ]
  },
  "nihilist": {
    "food": [
      "Ate all of it. Said nothing mattered. Licked the bowl."
    ],
    "fuss": [
      "Said it meant nothing and leaned into every second of it."
    ],
    "clean": [
      "Was cleaned. Pointed out it would only get dirty again. Sat in the cloth anyway."
    ]
  },
  "critic": {
    "food": [
      "Ate all of it. Two stars. Went back for the crumbs. Still two."
    ],
    "fuss": [
      "Three stars. It has never given three. It looked shaken."
    ],
    "clean": [
      "Reviewed the cloth. One star. Reviewed being warm afterwards. Did not publish."
    ]
  },
  "freegan": {
    "food": [
      "Ate the crumb from under the bowl first. Then the food. On principle."
    ],
    "fuss": [
      "Accepted the fuss. It was free."
    ],
    "clean": [
      "Was washed. Asked where the water had been first."
    ]
  },
  "cult": {
    "food": [
      "Set a crumb aside before eating. For Thursday."
    ],
    "fuss": [
      "Went still under your hand and said something quietly. It was thank you, in the voice."
    ],
    "clean": [
      "Was rinsed. Called it a rite. Asked for a candle. Got a towel."
    ]
  },
  "martyr": {
    "food": [
      "Said it did not need it. Ate all of it. Said it again with its mouth full."
    ],
    "fuss": [
      "Sighed through the whole fuss. Sat where your hand had been for an hour after."
    ],
    "clean": [
      "Endured the cloth. Loudly. Then went quiet and warm and did not mention it."
    ]
  },
  "bones": {
    "food": [
      "Ate round the hard bit and kept the hard bit. For the collection."
    ],
    "fuss": [
      "Leaned in. Something in it clicked. It said that was normal."
    ],
    "clean": [
      "Was washed. A small tooth came out in the water. Not its own. It has gone back in the set."
    ]
  },
  "taxidermy": {
    "food": [
      "Ate carefully, so as not to spoil the line of its jaw."
    ],
    "fuss": [
      "Held very still under your hand. Said you had an excellent touch. Asked where you learned it."
    ],
    "clean": [
      "Enjoyed being dried. Asked what you used. Wrote it down."
    ]
  },
  "spiteful": {
    "food": [
      "Ate. Crossed nothing off. Ate the rest."
    ],
    "fuss": [
      "Permitted it. Made a mark somewhere afterwards. The mark was small and kind."
    ],
    "clean": [
      "Was wiped. Kept the cloth in view for the rest of the day. Warm, though."
    ]
  },
  "terminal": {
    "food": [
      "Ate what it called its last meal. All of it. Asked about seconds."
    ],
    "fuss": [
      "Went limp with a small sigh. Opened one eye to make sure you were watching it go."
    ],
    "clean": [
      "Said the water was the end. Came out pink and shining and very much not ended."
    ]
  },
  "gossip": {
    "food": [
      "Ate, and told the bowl what it thought of the food. The bowl has told the spoon."
    ],
    "fuss": [
      "Purred, then told the lamp you had done it. The lamp already knew."
    ],
    "clean": [
      "Was cleaned. Told the cloth something about you. The cloth is discreet."
    ]
  }
};

export const DRAWN_NOTES = [
  "Has found a wobble in its outline. Calls it a hip.",
  "Would like to know whether you were finished when you pressed Save.",
  "Is proud of the bit you almost erased.",
  "Tried to stand up straight. The line does not go that way.",
  "Has decided the uneven side is its good side.",
  "You drew it. It has been drawing conclusions.",
  "Practised looking intimidating. Still recognisably your doodle.",
  "Its outline wobbled. It checked whether you were holding the pen.",
  "Has no idea what it is. Neither of you is bringing it up.",
  "Caught its reflection. Recognised the effort."
];
