...
export function marketErrandMarkup(state,m,trade,selection,panel,{workspace,stage,select}){
  ...
  const primary=atEnd?'':ready&&(full||short)?button(full?'Deliver to free two spaces':'Deliver for 4 buttons','market-panel','data-panel="requests"'):
    ...
    blocked?button(m.step===5?'Pass final stall':'Pass this stall','market-pass'):
  ...
  else actions='<p class="market-dock-selection">'+(returned?'Return '+esc(returned.name)+' for 1 button. ':'')+'Selected: '+esc(selected.name)+'</p>'+primary+'<div class="market-next-actions">'+button('Check errands'+(ready?' · pair ready':''),'market-panel','data-panel="requests"')+(blocked?'':button(m.step===5?'Pass final stall':'Pass this stall','market-pass'))+'</div>'+(full?'<p>'+(blocked?'No pair matches and your return is used. Pass the remaining stalls, then bring your spare buttons home.':ready?'Deliver a matching pair to free two spaces.':'Your bag has no matching pair. Choose one object to return with your purchase.')+'</p>':'<p>'+(short?'Choose a cheaper object, deliver a pair, or pass.':'After buying: '+(available-selected.cost)+' button'+(available-selected.cost===1?'':'s')+' left.')+'</p>')+(afterPass<possible?'<p class="market-pass-warning">Passing this stall leaves at most '+afterPass+' of 3 errands possible.</p>':'');
