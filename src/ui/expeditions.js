import { masteryText, masteryUnlockText } from '../mastery-state.js';
...
function fieldPage(id) {
 const [route,number,branch]=id.split(':'),edition=Number(number);
 const main=missionStops(route,edition,2);
 return {label:'Route edition '+(edition+1)+(branch?' · detour':''),stops:branch?[main[0],...missionStops(route,edition^6,2).slice(1)]:main};
}
function fieldCounts(pages) {
 const detours=pages.filter(id=>id.endsWith(':detour')).length;
 return (pages.length-detours)+'/8 main routes · '+detours+'/8 detours';
}
export function projectBoard(l,now=Date.now()) {
 return '<section class="household-workshop" aria-label="Household projects"><div class="workshop-heading"><span class="eyebrow">Built by the household</span><h3>Bring the place to life.</h3><p>Expeditions recover the parts. You decide what gets built.</p></div><div class="project-grid">'+PROJECTS.map(p=>{
  ...
  const journal=pages.length?'<details class="project-field-guide"><summary>Read field notes · '+fieldCounts(pages)+'</summary>'+pages.map(id=>{const page=fieldPage(id);return '<article><b>'+esc(page.label)+'</b>'+page.stops.map(stop=>'<p><strong>'+esc(stop.title)+'</strong><br>'+esc(stop.text)+'</p>').join('')+'</article>';}).join('')+'</details>':'';
  return '<article class="project-card project-'+p.id+' '+(built?'is-built':'is-blueprint')+'"><div class="project-drawing">'+projectArt(p.id)+(built?'<div class="project-occupant" aria-hidden="true"></div>':'')+'<span>'+(built?'INSTALLED':'PLAN '+String(PROJECTS.indexOf(p)+1).padStart(2,'0'))+'</span></div><div class="project-card-copy"><h4>'+esc(p.name)+'</h4><p>'+esc(p.benefit)+'.</p><p class="project-field-pages">'+fieldCounts(pages)+' recorded</p><button class="btn" data-life="'+(built?'use-project':'project-mission')+'" data-id="'+p.id+'">'+esc(built?p.action:'Recover parts · '+Math.min(n,2)+'/2')+'</button><p class="project-reaction" role="status">'+(built&&l.day===dayKey(now)&&l.daily?.includes('project:'+p.id)?'Today’s care delivered. Play again whenever you like.':'')+'</p>'+journal+'</div></article>';
 }).join('')+'</div></section>';
}
...
function choiceMarkup(option,o) {
 ...
 if(!option.available){
  ...
  if(o.masteryTier>=2&&o.step===0&&choice===1)detail+=' · Opens a different route for stops 2 and 3';
 }
...
function planningMarkup(state,route,gear,lead,companion,dare) {
 ...
 const edition=life.outings%8,stops=missionStops(id,edition,2),pages=(life.trailPages||[]).filter(key=>key.startsWith(id+':')),pageKnown=(life.trailPages||[]).includes(id+':'+edition);
