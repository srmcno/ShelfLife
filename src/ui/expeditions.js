...
import { OUTING_DARES, outingPreview } from '../engine/life.js';
...
function planningMarkup(state,route,gear,lead,companion,dare) {
 ...
 const expertise=[...new Set(crew.flatMap(p=>['cute','menace','damp','mystique'].filter(stat=>(p.stats?.[stat]||0)>=7)))];
 ...
 const pack=crew.map((pet,i)=>{
  const supported=stops.map((stop,index)=>(pet.stats?.[stop.stat]||0)>=7?index+1:null).filter(Boolean);
  return '<article class="expedition-packed-resident"><div class="expedition-portrait" data-expedition-pet="'+i+'" aria-hidden="true"></div><div><small>'+(i?'Companion':'Lead resident')+'</small><b>'+esc(pet.name)+'</b><p>'+(supported.length?'1-nerve detours at '+(supported.length===1?'stop ':'stops ')+supported.join(' & '):'No specialist shortcut on this route.')+'</p></div></article>';
 }).join('');
 ...
 const crewBrief=crew.map(pet=>{
  const supported=stops.filter(stop=>(pet.stats?.[stop.stat]||0)>=7);
  return '<p><b>'+esc(pet.name)+'</b> '+(supported.length?'Reduces detour cost to 1 nerve at '+supported.map(stop=>esc(stop.title)).join(' and ')+'.':'No specialist shortcut on this route. Detours cost 2 nerve.')+' <small>Can gain '+Math.max(0,Math.min(8,100-(pet.needs?.fuss||0)))+' attention this trip.</small></p>';
 }).join('');
