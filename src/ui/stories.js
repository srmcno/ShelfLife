...
export function renderMuseum(state) {
  ...
  }).join('')+'</div><h3>Postcard album <small>Last six saved pictures</small></h3><div class="album">'+(s.postcards.length?s.postcards.map(p=>'<figure><img src="'+esc(p.image)+'" alt="'+esc(p.caption)+'"><figcaption>'+esc(p.caption)+'<small>'+date(p.at)+'</small></figcaption></figure>').join(''):'<p class="hint">Open today’s postcard and choose Keep in museum. A small picture is saved here; Back up preserves the album.</p>')+'</div><h3>Former residents</h3><div class="memorials">'+(memorials.length?memorials.map(p=>'<article><b>'+esc(p.name)+'</b><p>'+esc(p.grudges!=null?p.grudges+' grievances. '+(p.names||[]).map(n=>n.name).join(' → '):'Their old place is still on file.')+'</p><small>Left '+date(p.at)+'</small></article>').join(''):'<p class="hint">Nobody has left. This is not a suggestion.</p>')+'</div><h3>Names & grievances</h3>'+state.pets.map(p=>'<div class="memory-row"><b>'+esc(p.name)+'</b>'+((p.names||[]).length>1?'<span>'+esc(p.names.map(n=>n.name).join(' → '))+'</span>':'')+'<small>'+(p.grudges||0)+(p.grudges===1?' grievance · ':' grievances · ')+(p.arcadeRuns||0)+' arcade runs · '+(p.expeditions||0)+' expeditions'+'</small></div>').join('')+'<h3>The case archive</h3><div class="memory-timeline">'+(s.archive.length?s.archive.map(m=>'<article><small>'+date(m.at)+' · '+esc(m.kind)+'</small><b>'+esc(m.title)+'</b><p>'+esc(m.text)+'</p></article>').join(''):'<p class="hint">Cases, promises, visitors and truces leave their evidence here.</p>')+'</div>';
