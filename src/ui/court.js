...
function playableStage(game,finished=false) {
 const v=courtView(game),h=courtHearing(game),stamps={sustained:'CLEARED',overruled:'STILL POSSIBLE',conviction:'GUILTY',acquittal:'TRY AGAIN',inspect:'EVIDENCE BAGGED'},stamp=stamps[h.phase];
...
export function crimeExhibits(game) {
 return game.rules.map((rule,i)=>{
  ...
  return {id:i,source:source.id,title:game.evidenceTitles[i],description:source.description,clue:game.clues[i],collected:game.inspected.includes(source.id),records:game.suspects.map(s=>axes.map(a=>s.details[a]).join('; '))};
 });
}
...
function hearingPanel(game) {
 ...
 return '<div class="court-panel-heading"><span class="eyebrow">2 · Compare facts, clear innocents</span><h4 id="courtPanelTitle" tabindex="-1">Does this evidence rule them out?</h4></div>'+castPicker(game)+clueTabs(game)+'<div class="court-fact-pair"><article><span>THE CRIME REQUIRES</span><p>'+esc(e.collected?e.clue:'Inspect this exhibit first.')+'</p></article><article><span>'+esc(s.name.toUpperCase())+' · VERIFIED RECORD</span><p>'+esc(e.collected?e.records[v.witness]:'This observation is still sealed.')+'</p></article></div>'+(cleared(game,v.witness)?'<p class="court-deduction-feedback">✓ Cleared. One conflicting fact is enough. Call someone else.</p>':compared?'<p class="court-deduction-feedback">This clue fits their record. Try another clue; a single match does not prove guilt.</p>':'<p class="court-next-step">Different facts? Use this clue to clear them. Matching facts? Check the next clue.</p>')+'<details class="court-character-aside"><summary>The witness would like to say something</summary><p>“'+esc(s.defence)+'”</p><small>Unsworn commentary. Use the verified record above to decide the case.</small></details>';
}
function verdictPanel(game) {
 const v=courtView(game),s=game.suspects[v.witness];
 return '<div class="court-panel-heading"><span class="eyebrow">3 · Make your accusation</span><h4 id="courtPanelTitle" tabindex="-1">One suspect must fit every clue.</h4></div>'+castPicker(game)+'<h4 class="court-accused-name">Your accusation: '+esc(s.name)+'</h4><div class="court-verdict-ledger">'+crimeExhibits(game).map(e=>'<article><b>Clue '+(e.id+1)+'</b><p>'+esc(e.collected?e.clue:'Evidence not inspected.')+'</p><span><b>Their record:</b> '+esc(e.collected?e.records[v.witness]:'Sealed')+'</span></article>').join('')+'</div><p class="court-next-step">A wrong accusation gets a specific explanation and keeps the case open. Nobody loses care or trust.</p>'+button('Explain a clue','court-hint',game.hints.length>=game.rules.length?'disabled':'');
}
function courtActionBar(game) {
 ...
 if(v.chapter==='investigation'){
  ...
  status=isClear?'Cleared. Choose another suspect.':'Use a conflicting clue to clear this suspect.';
  ...
  status=isClear?'This suspect is cleared. Select someone else.':!all?'Inspect all the crime clues before accusing.':'Check the two columns. Do all their facts fit the crime?';
