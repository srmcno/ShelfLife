...
import { courtEvidence, courtClueHelp, courtClearingReason, courtComparisonState, courtScore } from '../engine/court.js';
...
function playableStage(game,finished=false) {
 ...
 const hotspots=!finished&&v.chapter==='investigation'?crimeExhibits(game).map((e,i)=>'<button class="court-hotspot hotspot-'+i+' '+(e.collected?'is-bagged':'')+'" data-life="court-clue" data-clue="'+i+'" aria-label="Inspect Exhibit '+(i+1)+': '+esc(e.title)+'">'+curioSVG(evidenceShape(e,i))+'<span>'+(e.collected?'✓':i+1)+'</span></button>').join(''):'';
...
function clueTabs(game) {
 const v=courtView(game);
 return '<div class="court-clue-tabs" role="group" aria-label="Crime evidence">'+crimeExhibits(game).map(e=>button('<span>'+(e.collected?'✓':e.id+1)+'</span>'+esc(e.title),'court-clue','data-clue="'+e.id+'" aria-label="'+(e.collected?'Review':'Inspect')+' clue '+(e.id+1)+': '+esc(e.title)+'" aria-pressed="'+(v.statement===e.id)+'"')).join('')+'</div>';
}
function castPicker(game) {
 const v=courtView(game);
 return '<div class="court-cast-picker" role="group" aria-label="Call a suspect">'+game.suspects.map((s,i)=>button('<span id="suspectArt'+i+'" class="court-cast-art" aria-hidden="true"></span><b>'+esc(s.name)+'</b><small>'+(cleared(game,i)?'Cleared':v.witness===i?'On the stand':'Call to stand')+'</small>','court-call','data-choice="'+i+'" aria-pressed="'+(v.witness===i)+'"')).join('')+'</div>';
}
...
function investigationPanel(game) {
 ...
 return '<div class="court-panel-heading"><span class="eyebrow">1 · Read the physical evidence</span><h4 id="courtPanelTitle" tabindex="-1">What must be true of the culprit?</h4></div>'+clueTabs(game)+'<article class="court-proof-sheet"><span class="eyebrow">Exhibit '+(e.id+1)+' · '+esc(e.title)+'</span>'+(e.collected?'<p class="court-proof">'+esc(e.clue)+'</p><p class="hint">This is a verified crime clue. A suspect with a conflicting observation is innocent.</p>':'<p>'+esc(e.description)+'</p><p class="hint">Inspect this exhibit to reveal its crime clue.</p>')+'</article>'+clueHelp(game,e)+'<p class="court-howto"><b>The whole case:</b> collect '+game.rules.length+' clues → clear anyone who conflicts with a clue → accuse the one who fits them all. No timer. No guessing which witness is lying.</p>';
}
function hearingPanel(game) {
 ...
 return '<div class="court-panel-heading"><span class="eyebrow">2 · Compare facts, clear innocents</span><h4 id="courtPanelTitle" tabindex="-1">Does this evidence rule them out?</h4></div>'+courtMatrixMarkup(game)+clueTabs(game)+'<div class="court-fact-pair"><article><span>THE CRIME REQUIRES</span><p>'+esc(e.collected?e.clue:'Inspect this exhibit first.')+'</p></article><article><span>'+esc(s.name.toUpperCase())+' · VERIFIED RECORD</span><p>'+esc(e.collected?e.records[v.witness]:'This observation is still sealed.')+'</p></article></div>'+(cleared(game,v.witness)?deductionFeedback(game,v.witness):compared?'<p class="court-deduction-feedback">This clue fits their record. Try another clue; a single match does not prove guilt.</p>':'<p class="court-next-step">Test the whole rule. If their record breaks it, you can clear them. Otherwise they remain possible.</p>')+clueHelp(game,e)+'<details class="court-character-aside"><summary>The witness would like to say something</summary>'+(s.memory?'<p>'+esc(s.memory)+'</p>':'')+'<p>'+esc(s.defence)+'</p><small>Unsworn commentary. Use the verified record above to decide the case.</small></details>';
}
function verdictPanel(game) {
 const v=courtView(game),s=game.suspects[v.witness];
 return '<div class="court-panel-heading"><span class="eyebrow">3 · Make your accusation</span><h4 id="courtPanelTitle" tabindex="-1">One suspect must fit every clue.</h4></div>'+courtMatrixMarkup(game)+'<h4 class="court-accused-name">'+(cleared(game,v.witness)?'Cleared suspect: ':'Your accusation: ')+esc(s.name)+'</h4>'+deductionFeedback(game,v.witness)+'<div class="court-verdict-ledger">'+crimeExhibits(game).map(e=>'<article><b>Clue '+(e.id+1)+'</b><p>'+esc(e.collected?e.clue:'Evidence not inspected.')+'</p><span><b>Their record:</b> '+esc(e.collected?e.records[v.witness]:'Sealed')+'</span>'+clueHelp(game,e)+'</article>').join('')+'</div><p class="court-next-step">A wrong accusation costs 15 case points and explains the conflicting evidence. The case stays open. Nobody loses care or trust.</p>';
}
function courtActionBar(game) {
 const v=courtView(game),exhibits=crimeExhibits(game),e=exhibits[Math.min(v.statement,exhibits.length-1)],all=exhibits.every(e=>e.collected),next=exhibits.find(x=>!x.collected),isClear=cleared(game,v.witness);
 let actions,status;
 if(v.chapter==='investigation'){
  ...
  status=!e.collected?'Inspect this clue before comparing the suspect’s record.':isClear?(game.suspects.filter((_,i)=>!cleared(game,i)).length===1?'One suspect remains. Make an accusation when you are ready.':'Cleared. Choose another suspect.'):game.version===3?'Clear only with a contradiction. An unsupported argument costs 5 case points; hints are free.':'Use a conflicting clue to clear this suspect.';
  actions=(!e.collected?button('Inspect clue '+(e.id+1),'court-clue','data-clue="'+e.id+'"'):button(isClear?'Suspect cleared':tested?'Clue already checked':'Clear with clue '+(e.id+1),'court-compare',!isClear&&!tested?'':'disabled'))+button('Make an accusation →','court-chapter','data-chapter="verdict" '+(all?'':'disabled'));
 }else{
...
export function courtFinishedMarkup(game,result,wins) {
 ...
 return '<div class="court-case court-playable court-deduction court-finished"><header class="court-play-header"><div><span class="eyebrow">Judgment delivered</span><h3 id="courtVerdictTitle" tabindex="-1">'+esc(result.rank||game.rank||'Case closed')+'</h3></div></header><div class="court-workspace">'+playableStage(game,true)+'<section class="court-action-panel" aria-label="Verdict and sentence"><div class="court-result"><h4>The evidence has teeth.</h4><div class="court-verdict-metrics"><b>'+result.score+'<span>case points</span></b><b>'+(result.stats.eliminations||0)+' / '+(game.suspects.length-1)+'<span>innocents cleared</span></b><b>'+result.stats.mistakes+'<span>missed arguments</span></b></div><p>'+esc(result.text)+'</p><p>'+esc(courtRewardSummary(result))+' '+wins+' case'+(wins===1?'':'s')+' solved</p></div><details class="court-sentence-details"><summary>The sentence and the last word</summary><div class="court-transcript">'+(result.dialogue||[]).map(l=>'<p><b>'+esc(l.speaker)+'</b><span>'+esc(l.text)+'</span></p>').join('')+'</div></details><details class="court-review"><summary>Why the verdict stands</summary><div class="court-verdict-reasons">'+(result.reasons||[]).map(r=>'<article><b>'+esc(r.name)+'</b><p>'+esc(r.text)+' '+esc(r.explanation)+'</p></article>').join('')+'</div></details></section></div><footer class="court-action-bar"><p>Case closed. The witnesses are already revising their memoirs.</p><div>'+button('Another case','court-new')+button('Close case','close')+'</div></footer></div>';
}
