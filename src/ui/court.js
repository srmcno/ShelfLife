...
import { courtEvidence, courtClueHelp, courtClearingReason, courtScore } from '../engine/court.js';
...
function clueTabs(game) {
 const v=courtView(game);
 return '<div class="court-clue-tabs" role="group" aria-label="Crime evidence">'+crimeExhibits(game).map(e=>button('<span>'+(e.collected?'✓':e.id+1)+'</span>'+esc(e.title),'court-clue','data-clue="'+e.id+'" aria-pressed="'+(v.statement===e.id)+'"')).join('')+'</div>';
}
...
function courtActionBar(game) {
 ...
 if(v.chapter==='investigation'){
  ...
  status=isClear?(game.suspects.filter((_,i)=>!cleared(game,i)).length===1?'One suspect remains. Make an accusation when you are ready.':'Cleared. Choose another suspect.'):game.version===3?'Clear only with a contradiction. An unsupported argument costs 5 case points; hints are free.':'Use a conflicting clue to clear this suspect.';
  actions=button(isClear?'Suspect cleared':tested?'Clue already checked':'Clear with clue '+(e.id+1),'court-compare',e.collected&&!isClear&&!tested?'':'disabled')+button('Make an accusation →','court-chapter','data-chapter="verdict" '+(all?'':'disabled'));
 }else{
  ...
  actions=button('Keep investigating','court-chapter','data-chapter="hearing"')+button('Accuse '+esc(game.suspects[v.witness].name),'court-file',all&&!isClear?'':'disabled');
 }
