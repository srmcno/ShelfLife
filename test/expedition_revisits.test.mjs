...
import {missionPlan,missionResult,projectBoard} from '../src/ui/expeditions.js';
...
test('an early single-part return banks the part without phantom recovery, points, or a completed field note',()=>{
 ...
 assert.ok(result.log.slice(1).every(line=>line.includes('unexplored')));assert.match(missionResult(result),/Unvisited stops gave no nerve, points or field note/);
...
test('field notes are bounded, survive reload, name visited encounters and never repay a replay',()=>{
 ...
 for(let edition=0;edition<8;edition++){
  ...
  const view=outingSnapshot(s);assert.equal(view.result.page,'drawer:'+edition);assert.equal(view.result.pageFresh,true);assert.match(missionResult(view),/\+2 discoveries/);s=normalizeState(s);assert.equal(s.life.trailPages.length,edition+1);finishOuting(s);
 }
...
test('crew and tool previews name the actual advantages and capped attention',()=>{
 const s=fixture(),plan=missionPlan(s,'drawer','thread','p','q','');
